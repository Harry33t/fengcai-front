...
import { getMenuListRequest, requestUserMenus, requestUserInfo } from '@/service/api'
...
async function processFrontendMenu(router: Router): Promise<void> {
  ...
  const userStore = useUserStore()
  const roles = userStore.info.roles || ['R_USER'] // 默认角色

  console.log('[processFrontendMenu] 用户角色:', roles)
  console.log('[processFrontendMenu] 用户信息:', userStore.info)
  console.log('[processFrontendMenu] 原始菜单列表:', menuList)
  console.log('[processFrontendMenu] 原始菜单数量:', menuList.length)

  // 临时禁用角色过滤，直接使用所有菜单
  const filteredMenuList = menuList // 不进行角色过滤
  
  console.log('[processFrontendMenu] 过滤后菜单列表:', filteredMenuList)
  console.log('[processFrontendMenu] 过滤后菜单数量:', filteredMenuList.length)
